...
const ReturnBook = () => {
  ...
  useEffect(() => {
    async function fetchBookTrans() {
      const response = await fetch("https://localhost:7231/api/BookTrans");
  ...
  useEffect(() => {
    if (booksTrans.length > 0) {
      booksTrans.forEach(async (i: any) => {
        const response =
          await fetch(`https://localhost:7231/api/Books/${i.bookId}
        `);
  ...
  async function submitHandler(event: React.MouseEvent) {
    ...
    try {
      const response = await fetch(
        `https://localhost:7231/api/BookTrans/${bookTransData.bookTranId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        const bookResponse = await fetch(
          `https://localhost:7231/api/Books/${bookTransData.bookId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
